...
export default function Player() {
  ...
  const name = isTrack ? current.track!.name : current.station!.name
  const subtitle = isTrack ? current.track!.artist_name : (current.station!.tags || current.station!.country || '')
  const image = isTrack
    ? (current.track!.image || current.track!.album_image)
    : current.station!.favicon
  const progress = isTrack && duration > 0 ? (currentTime / duration) * 100 : 0
  ...
  return (
    <div className="flex-shrink-0 border-t border-white/5 bg-surface/95 backdrop-blur">
      ...
      <div
        className="flex items-center gap-3 px-4 py-2 cursor-pointer safe-bottom"
      ...
      >
        <div className="w-10 h-10 rounded-lg bg-surface-light flex items-center justify-center flex-shrink-0 overflow-hidden">
          {image ? (
            ...
            <span className="text-lg">{isTrack ? '🎵' : '📻'}</span>
          )}
        </div>
        ...
        <div className="flex items-center gap-2">
          ...
          <button
            ...
            className="w-9 h-9 flex items-center justify-center rounded-full bg-accent text-surface-dark hover:bg-accent-hover transition-colors"
          >
            {isPlaying ? (
