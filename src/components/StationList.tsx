...
interface Props { stations: RadioStation[]; loading?: boolean; emptyMessage?: string }

export default function StationList({ stations, loading, emptyMessage = '没有找到电台' }: Props) {
  if (loading) {
  ...
  return <div className="grid grid-cols-2 gap-3">{stations.map(s => <StationCard key={s.stationuuid} station={s} />)}</div>
}
